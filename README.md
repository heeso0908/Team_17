...
스타벅스 CRM 팀의 일원이 되어 고객에게 다양한 프로모션을 제공하며,  
고객의 반응과 실제 구매 데이터를 기반으로 마케팅 전략을 최적화

본 프로젝트는 스타벅스 고객 데이터(`profile`, `portfolio`, `transcript`)를 활용하여  
**프로모션 반응, 구매 전환, 고객 특성별 행동 차이**를 분석하고,  
이를 바탕으로 **마케팅 인사이트 도출 및 Tableau 대시보드 제작**을 목표로 한다.

### 배경
마케팅에서는 단순히 프로모션을 많이 발송하는 것보다  
**어떤 고객에게 / 어떤 오퍼를 / 어떤 채널로 전달해야 실제 구매로 이어지는지**를 파악하는 것이 중요하다.

이를 위해 고객 데이터, 프로모션 데이터, 행동 로그 데이터를 연결하여  
**캠페인 반응 구조와 구매 전환 흐름**을 분석한다.
